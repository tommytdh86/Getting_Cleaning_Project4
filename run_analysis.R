...
# Setting variable to local data file
dataFileZIP <- "./getdata-projectfiles-UCI-HAR-Dataset.zip"

#  Setting variable to directory
dirFile <- "./UCI HAR Dataset"

# Naming directory and filename of the clean/tidy data:
...
# Download the file (. ZIP), if it does not exist
...
names(tidyDataAVGSet)[2] <- "Activity"# Created txt (tidy data set) in diretory

# Create txt in diretory
write.table(tidyDataSet, tidyDataFile)

# Create txt (AVG) in diretory
